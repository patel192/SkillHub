...
export const CourseQuiz = () => {
  ...
  const handleBulkDeploy = async () => {
    ...
    try {
      if (bulkData.trim().startsWith("[") || bulkData.trim().startsWith("{")) {
        ...
        items = lines.map((l) => {
          const [q, o1, o2, o3, o4, cIdx, pts] = l
            .split(",")
            .map((s) => s.trim());
          return {
            question: q,
    ...
    for (let i = 0; i < items.length; i++) {
      try {
        ...
        toast.loading(`Synchronizing Matrix: ${i + 1}/${items.length}`, {
          id: tid,
        });
  ...
  const downloadTemplate = (type) => {
    ...
    if (type === "json") {
      ...
      content =
        "Question, Opt1, Opt2, Opt3, Opt4, CorrectIdx(0-3), Points\nSample Question, Yes, No, Maybe, N/A, 0, 10";
  ...
  `}
            ...
            >
              <div className="pb-10 border-b border-border space-y-4">
                ...
                <p className="text-[10px] font-bold uppercase tracking-widest opacity-40">
                  Define question logic and scoring parameters below.
                </p>
              </div>
              ...
              <div className="space-y-8">
                ...
                <div className="space-y-4">
                  <label className="text-[10px] font-black uppercase tracking-widest opacity-40 ml-1">
                    Response Options (Select Correct Solution)
                  </label>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {newQuestion.options.map((opt, i) => (
                      <div
                        key={i}
                        className={`p-4 rounded-3xl border transition-all flex items-center gap-4 ${opt.isCorrect ? "bg-emerald-500/10 border-emerald-500/30" : "bg-surface border-border/50 focus-within:border-brand/40"}`}
                      >
                        <button
                          onClick={() => {
                            const updated = newQuestion.options.map(
                              (o, idx) => ({ ...o, isCorrect: idx === i }),
                            );
                            setNewQuestion({
                              ...newQuestion,
                              options: updated,
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex items-center gap-4">
                      <input
                        ...
                        className="flex-1"
                      />
                      <span className="w-12 text-center font-black text-brand bg-brand/10 py-1 rounded-lg border border-brand/20">
                        {newQuestion.points}pt
                      </span>
                    </div>
                  </div>
                  <div className="flex flex-col justify-center">
                    <p className="text-[9px] font-black uppercase tracking-widest opacity-30 leading-relaxed">
                      This value defines the raw point weight of this assessment
                      node within the global curriculum matrix.
                    </p>
                  </div>
                </div>
    ...
    "
            ...
            <motion.div
              ...
              className="w-full lg:max-w-3xl lg:mx-auto space-y-12 px-4 sm:px-6"
            >
              <div className="space-y-6 pb-12 border-b border-border/50">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <span className="px-2.5 py-1 rounded-lg bg-emerald-500/10 text-emerald-500 text-[9px] font-black uppercase tracking-[0.2em] border border-emerald-500/20">
                      Assessment Node Live
                    </span>
                    <span className="text-[10px] font-bold opacity-30 uppercase tracking-widest flex items-center gap-2">
                      <Award size={12} /> {selectedQuestion.points} Points
                      Weight
                    </span>
                  </div>
                ...
                <h1
                  className="text-4xl lg:text-5xl font-black tracking-tighter leading-tight"
                  style={{ fontFamily: "'Fraunces', serif" }}
                >
              ...
              <div className="grid grid-cols-1 gap-4">
                {selectedQuestion.options.map((opt, i) => (
                  <div
                    key={i}
                    className={`p-6 rounded-[2rem] border transition-all flex items-center gap-6 ${opt.isCorrect ? "bg-emerald-500/10 border-emerald-500/30" : "bg-surface border-border/50"}`}
                  >
                    <div
                      className={`w-8 h-8 rounded-full border-2 flex items-center justify-center transition-all ${opt.isCorrect ? "bg-emerald-500 border-emerald-500 text-white shadow-[0_0_15px_rgba(16,185,129,0.4)]" : "border-border"}`}
                    >
                      {opt.isCorrect ? (
                        <CheckCircle size={16} />
                      ) : (
                    ...
                    <span
                      className={`text-lg font-bold ${opt.isCorrect ? "text-emerald-400" : "opacity-60"}`}
                    >
                      {opt.text}
                    </span>
                    {opt.isCorrect && (
                      <span className="ml-auto text-[9px] font-black uppercase tracking-widest text-emerald-500/60 bg-emerald-500/5 px-3 py-1 rounded-full">
                        Correct Logic
                      </span>
                    )}
                  </div>
              ...
              <div className="p-8 rounded-[2.5rem] bg-surface border border-border flex items-center justify-between shadow-sm">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h4 className="text-[10px] font-black uppercase tracking-widest opacity-30 mb-0.5">
                      Assessment Security
                    </h4>
                    <p className="text-xs font-bold uppercase tracking-tight">
                      Verified & Distributed Node
                    </p>
                  </div>
                </div>
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-center p-8">
            <div className="p-12 rounded-[4rem] bg-surface shadow-2xl border border-border flex flex-col items-center max-w-sm">
              ...
              <p className="text-[10px] font-bold uppercase tracking-widest opacity-40 mt-2">
                Initialize assessment logic by deploying a new question node to
                the grid.
              </p>
              <button
                onClick={() => setAdding(true)}
  ...
  >
    <div className="flex items-center justify-between gap-3">
      <div className="flex items-center gap-2">
        ...
        <span className="text-[8px] font-black text-accent bg-accent/10 px-1.5 py-0.5 rounded border border-accent/20">
          W:{question.points}
        </span>
      </div>
      <button
    ...
    <h4
      className={`text-[11px] font-black tracking-tight line-clamp-2 leading-tight ${isActive ? "text-brand" : "opacity-70"}`}
    >
      {question.question}
    </h4>
  </motion.div>
