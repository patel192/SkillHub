...
const Toast = ({ message, type, onClose }) => (
  <motion.div
    ...
    style={{
      ...
      color: "var(--bg)",
      boxShadow: `0 10px 40px ${type === 'error' ? 'rgba(239,68,68,0.3)' : 'rgba(16,168,128,0.3)'}`
    }}
  >
...
export const AdminSettings = () => {
  ...
  const renderAccountTab = () => (
    <div className="space-y-6">
      <SectionCard title="Administrator Profile" description="Update your personal details">
        <div className="flex items-center gap-4 mb-6">
          ...
          <div>
            <p className="font-semibold" style={{ color: "var(--text)" }}>{userData.fullname || 'Admin'}</p>
            <p className="text-sm" style={{ color: "var(--text-muted)" }}>Administrator since {new Date(userData.createdAt).toLocaleDateString()}</p>
          </div>
        </div>
        ...
        <div className="space-y-1.5">
          ...
          <textarea value={userData.bio} onChange={(e) => setUserData(prev => ({ ...prev, bio: e.target.value }))} placeholder="Admin bio..." rows={3}
            className="w-full p-3 rounded-xl outline-none resize-none shadow-sm" style={{ background: "var(--surface2)", border: `1px solid ${"var(--border)"}`, color: "var(--text)" }} />
        </div>
  ...
  const renderPrivacyTab = () => (
    <SectionCard title="Privacy Settings" description="Manage your visibility">
      <div className="space-y-6">
        ...
        <CustomToggle label="Show activity on public records" checked={localSettings.showActivity} onChange={() => setLocalSettings(prev => ({ ...prev, showActivity: !prev.showActivity }))} />
  ...
  const renderAppearanceTab = () => (
    <SectionCard title="Appearance" description="Customize dashboard theme">
      <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
            {[ 
              ...
              <motion.button 
                ...
                className="group relative h-24 rounded-2xl overflow-hidden border-2 transition-all flex flex-col items-center justify-center"
                style={{ 
                  background: bg, 
              ...
              >
                <div className="w-8 h-8 rounded-full shadow-lg flex items-center justify-center mb-2" style={{ background: primary }}>
                  {localSettings.theme === key && <Check size={16} className="text-white" />}
                </div>
                <span className="text-[10px] font-black uppercase tracking-widest" style={{ color: localSettings.theme === key ? primary : '#888' }}>
                  {label}
                </span>
                {localSettings.theme === key && (
                  <motion.div layoutId="active-theme-admin" className="absolute inset-0 border-2" style={{ borderColor: primary }} />
                )}
              </motion.button>
            ))}
          </div>
  ...
  const renderNotificationsTab = () => (
    <SectionCard title="Notification Preferences" description="Admin alerts and updates">
      <div className="space-y-6">
        <div className="space-y-4">
          <h4 className="font-semibold text-sm" style={{ color: "var(--text)" }}>Channels</h4>
        ...
        <div className="space-y-4 pt-4 border-t" style={{ borderColor: "var(--border)" }}>
          <h4 className="font-semibold text-sm" style={{ color: "var(--text)" }}>Content</h4>
          <CustomToggle label="System health and course reports" checked={localSettings.courseUpdates} onChange={() => setLocalSettings(prev => ({ ...prev, courseUpdates: !prev.courseUpdates }))} />
          <CustomToggle label="User verification alerts" checked={localSettings.achievementAlerts} onChange={() => setLocalSettings(prev => ({ ...prev, achievementAlerts: !prev.achievementAlerts }))} />
        </div>
        <div className="pt-4 border-t" style={{ borderColor: "var(--border)" }}>
          <SaveButton onClick={handleNotificationsSave} loading={saving}>Update Settings</SaveButton>
        </div>
      </div>
    </SectionCard>
  );
  ...
  if (settingsLoading) {
    return (
      <div className="flex items-center justify-center h-screen">
        <motion.div animate={{ rotate: 360 }} transition={{ duration: 1, repeat: Infinity, ease: "linear" }} className="w-10 h-10 border-4 border-t-transparent rounded-full" style={{ borderColor: C.brand }} />
      </div>
    );
  }

  return (
    <div className="p-6 max-w-4xl mx-auto space-y-8" style={{ color: "var(--text)" }}>
      <AnimatePresence>{toast && <Toast {...toast} onClose={() => setToast(null)} />}</AnimatePresence>

      <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
        <h1 className="text-3xl font-black" style={{ fontFamily: "'Fraunces', serif" }}>Admin <span style={{ color: C.brand }}>Settings</span></h1>
        <p className="text-sm opacity-50 mt-1">Manage administrative preferences and profile information</p>
      </motion.div>

      <div className="flex flex-wrap gap-2">
        {tabs.map((tab) => (
          <motion.button key={tab.key} whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} onClick={() => setActiveTab(tab.key)}
            className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-bold tracking-tight transition-all"
            style={{ 
              background: activeTab === tab.key ? "var(--surface2)" : 'transparent', 
              border: `1px solid ${activeTab === tab.key ? C.brand : "var(--border)"}`, 
              color: activeTab === tab.key ? C.brand : "var(--text-muted)" 
            }}>
