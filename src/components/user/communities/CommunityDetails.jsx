...
const useCommunity = (communityId) => {
  ...
  const updateCommunity = async () => {
    try {
      ...
      await apiClient.put(`/communities/${communityId}`, {
        ...editData,
        userId,
      });
...
const useMembership = (community, communityId, refreshCommunity) => {
  ...
  const isAdmin = useMemo(() => {
    ...
    return community.members.some(
      (m) => extractId(m.userId) === userId && m.role === "admin",
    );
  }, [community, userId]);
  ...
  const removeMember = async (memberId) => {
    try {
      await apiClient.patch(`/communities/${communityId}/leave`, {
        userId: memberId,
      });
  ...
  const promoteMember = async (memberId) => {
    try {
      await apiClient.patch(`/communities/${communityId}/promote`, {
        userId: memberId,
      });
  ...
  const addMember = async (memberId) => {
    try {
      await apiClient.patch(`/communities/${communityId}/join`, {
        userId: memberId,
      });
...
const usePosts = (communityId, refreshCommunity) => {
  ...
  const fetchPosts = useCallback(async () => {
    try {
      ...
      const res = await apiClient.get(
        `/communities/${communityId}/posts?sort=new&limit=50`,
      );
  ...
  const pinPost = async (postId, isPinned) => {
    try {
      ...
      await apiClient.patch(`/communities/${communityId}/${action}`, {
        postId,
      });
...
const useComments = (communityId, refreshPosts) => {
  ...
  const addReply = async (postId, commentId, content) => {
    try {
      await apiClient.post(`/posts/${postId}/comment/${commentId}/reply`, {
        userId,
        content,
      });
          ...
          <div>
            ...
            >
              <div className="flex items-center gap-1.5">
                ...
                <span>
                  <AnimatedCounter value={memberCount} /> members
                </span>
              </div>
              <div className="flex items-center gap-1.5">
    ...
    >
      ...
      <div className="p-5">
        ...
        <div
          ...
          style={{ borderTop: `1px solid rgba(22,168,128,0.1)` }}
        >
          <motion.button
            whileTap={{ scale: 0.9 }}
            onClick={onLike}
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <img
                            src={
                              user.avatar ||
                              `https://ui-avatars.com/api/?name=${user.fullname}&background=16A880&color=fff`
                            }
                            alt={user.fullname}
...
export const CommunityDetails = () => {
  ...
  const {
    ...
    addMember,
    userId,
  } = useMembership(community, communityId, refreshCommunity);
  const {
    posts,
    ...
    toggleLike,
    pinPost,
  } = usePosts(communityId, refreshCommunity);
            ...
            <div className="space-y-4">
              {posts.map((post) => (
                <PostCard
                  ...
                  onReply={(commentId, text) =>
                    addReply(post._id, commentId, text)
                  }
                />
              ))}
            </div>
